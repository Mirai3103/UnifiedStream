...
**Open, and to be settled before task 2 is implemented:** whether the two-channel rule holds across VB-CABLE releases, and whether any release ships a second stereo render endpoint that would make the candidate list ambiguous. Only 3.3.1.7 has been examined. Verifying against one older and one newer release is a cheap spike and it is the difference between a rule and a guess.
